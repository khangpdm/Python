...
function editUser(index) {
    if (index >= 0 && index < users.length) {
        ...
        document.getElementById('editUser').style.display = 'block'; // Hiện form chỉnh sửa
    } else {
...
function saveUserChanges(index, username, password) {
    if (index >= 0 && index < users.length) {
        ...
        updateLocalStorage();
        displayUsers();
        document.getElementById('editUser').style.display = 'none'; // Ẩn form chỉnh sửa
    } else {
        console.error('Invalid user index');
    }
}
...
// Hiển thị người dùng ngay khi tải trang
displayUsers();